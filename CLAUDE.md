...
**Copyright note:** almost every book here is an older work with a plausible (if unverified) public-domain argument. One is not: `raw/general/Marc S. Micozzi - Fundamentals of Complementary and Alternative Medicine - 5th Edition.epub` is a modern (2014), actively in-print, commercially published Elsevier textbook, added at the user's explicit request after being flagged as a copyright concern. Don't treat it as precedent for adding other modern copyrighted works without the same explicit, informed confirmation — and think carefully before this repo is ever pushed somewhere public while that file remains in `raw/`.
...
- `micozzi-fundamentals-of-complementary-and-alternative-medicine-5th` isn't from Internet Archive at all — the user supplied a born-digital PDF (real text, a 468-entry embedded TOC, 167 images) that was converted to EPUB with a one-off script (not part of this repo) mimicking the standard `<title>Page N</title>` + one-`<p>`-per-page layout, specifically so it would pass through the *ordinary* `parse_source_epub()` path unmodified. That conversion script no longer exists anywhere in this repo; if this book ever needs reprocessing from its original PDF, it must be rebuilt (page text + TOC + images) from scratch, since only page text survived into `raw/general/`'s EPUB and then into `processed/`.
