...
- **`madhava-nidana-vol-1-lochan`** (30), **`bhavaprakasha-vol-1-srikantha-murthy-2001`** (35), **`adams-practical-guide-to-homeopathic-treatment-1913`** (7), **`majumdar-homoeopathic-chikitsa-prakaran-bengali`** (23) — each needed a reverse-engineered regex plus book-specific number-repair logic (OCR-dropped digits, roman numerals, or Bengali ordinal words) to separate real chapter openings from table-of-contents listings and inline cross-references. Coverage is best-effort for these — some chapters have no recoverable heading at all.
- **`ernst-oxford-handbook-of-complementary-medicine`** (7) — its "CHAPTER N" headers repeat as a running page header on every page of a chapter rather than marking just the opening page, so extraction keeps only the first occurrence per number; all 7 chapters recovered, not best-effort.

Every other book was checked too (not just these) and found to genuinely lack chapter/heading structure recoverable from its OCR text or source markup — most are alphabetical materia medica/repertory references with no narrative chapters to begin with. One partial exception: `pandey-homoeopathic-practice-of-medicine` (a Bengali facsimile) does have real chapter structure using the same Bengali-ordinal-word convention as Majumdar above, but its Tesseract OCR is too degraded to recover more than 2 of an estimated 20+ chapters, so it was left unwired.
